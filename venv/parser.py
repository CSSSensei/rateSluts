...
async def fetch_posts(session, vk_token, version, domain, offset):
...
async def get_posts(parameters: dict) -> dict:
    photo_list = []
    flag = True
    offset = 0
    ...
    async with aiohttp.ClientSession() as session:
        while len(photo_list) < parameters.get('photo_amount', 5) and flag:
            data = await fetch_posts(session, vk_token, version, parameters['domain'], offset)
            offset += 1
