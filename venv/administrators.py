...
async def get_admins():
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT id FROM admins_info WHERE active=True")
...
async def get_message_to_delete(user_id):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT trigger FROM admins_info WHERE id=?", (user_id,))
...
async def set_message_to_delete(user_id, message_id: str):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE admins_info SET trigger=? WHERE id=?", (message_id, user_id,))
        await db.commit()


async def set_admin(user_id):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT * FROM admins_info WHERE id=?", (user_id,))
        rows = await cursor.fetchall()
        if not rows:
            await db.execute("INSERT INTO admins_info (id, active, trigger, day_of_week, hour, minute, mode) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, True, '-1', '1, 2, 3, 4, 5, 6, 7', '20', '00', 1))

        else:
            await db.execute("UPDATE admins_info SET active=? WHERE id=?", (True, user_id,))
        await db.commit()


async def update_groupnames():
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT id FROM group_parser")
        rows = await cursor.fetchall()
        for row in rows:
            id = row[0]
            cursor = await db.execute("SELECT group_domain FROM group_parser WHERE id=?", (id,))
            domain = (await cursor.fetchone())[0]
            name, photo = api_group_name(domain)
            await asyncio.sleep(0.4)
            cursor = await db.execute("UPDATE group_parser SET name=?, cover=? WHERE group_domain=?", (name, photo, domain,))
            await db.commit()


async def add_to_queue_group_photo(user_id: int, num: str):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT extra2 FROM admins_info WHERE id=?", (user_id,))
        ...
        if queue is None or queue[0] == '' or queue[0] is None:
            ...
            queue.add(num)
            queue = ', '.join(queue)
        await db.execute("UPDATE admins_info SET extra2=? WHERE id=?", (queue, user_id,))
        await db.commit()
...
async def remove_from_admins_queue(user_id, num):
    async with aiosqlite.connect(adminDB) as db:
        if num==0:
            await db.execute("UPDATE admins_info SET extra2=? WHERE id=?", (None, user_id,))
            await db.commit()
            return
        cursor = await db.execute("SELECT extra2 FROM admins_info WHERE id=?", (user_id,))
        queue = await cursor.fetchone()
...
async def set_admin_mode(user_id, mode):
    async with aiosqlite.connect(adminDB) as db:
        ...
        rows = await cursor.fetchall()
        if not rows:
            await db.execute("INSERT INTO admins_info (id, active, mode) VALUES (?, ?, ?)",
                           (user_id, True, mode))
        else:
            await db.execute("UPDATE admins_info SET mode=? WHERE id=?", (mode, user_id,))
        await db.commit()
...
async def set_inactive(user_id: int):
    async with aiosqlite.connect(adminDB) as db:
        ...
        if not rows:
            ...
            await db.execute("UPDATE admins_info SET active=? WHERE id=?", (False, user_id,))
        await db.commit()
...
async def get_weekdays(user_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT day_of_week FROM admins_info WHERE id=?", (user_id,))
        rows = await cursor.fetchone()
...
async def change_weekdays(user_id: int, day: int):
    async with aiosqlite.connect(adminDB) as db:
        ...
        if weekdays is None:
            await db.execute("UPDATE admins_info SET day_of_week=NULL WHERE id=?", (user_id,))
        else:
            await db.execute("UPDATE admins_info SET day_of_week=? WHERE id=?", (weekdays, user_id,))
        await db.commit()


async def get_group_domain(group_id: Union[str, int]):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT group_domain FROM group_parser WHERE id=?", (group_id,))
...
async def get_group_sets(group_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT * FROM group_parser WHERE id=?", (group_id,))
        rows = await cursor.fetchone()
        if rows[7] is None:
            name, photo = await api_group_name(rows[1])  # Assuming api_group_name is an asynchronous function
            await db.execute("UPDATE group_parser SET name=?, cover=? WHERE id=?", (name, photo, group_id))
            await db.commit()
...
async def send_photos_by_id(num: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT * FROM group_photos WHERE id=?", (num,))
        rows = await cursor.fetchone()
        return rows
...
async def get_group_name(domain: Union[str, int]):
    async with aiosqlite.connect(adminDB) as db:
        if isinstance(domain, str):
            cursor = await db.execute("SELECT name FROM group_parser WHERE group_domain=?", (domain,))
            rows = await cursor.fetchone()
            if rows is None:
                return None
            else:
                if rows[0] is None:
                    name, photo = api_group_name(domain)
                    await db.execute("UPDATE group_parser SET name=?, cover=? WHERE group_domain=?", (name, photo, domain,))
                    await db.commit()
                    return name.replace('<','').replace('>','')
                return rows[0].replace('<','').replace('>','')
        else:
            cursor = await db.execute("SELECT name FROM group_parser WHERE id=?", (domain,))
            rows = await cursor.fetchone()
            if rows is None:
                return None
            else:
...
async def get_active_groups():
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT id FROM group_parser WHERE active=?", (True,))
        rows = await cursor.fetchall()
...
async def get_last_group():
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT MAX(id) FROM group_parser")
        rows = await cursor.fetchone()
        if rows is None:
            return 0
...
async def switch_active(group_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT active FROM group_parser WHERE id=?", (group_id,))
        active = (await cursor.fetchone())[0]
        await db.execute("UPDATE group_parser SET active=? WHERE id=?", (not active, group_id,))
        await db.commit()


async def switch_top_likes(group_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT top_likes FROM group_parser WHERE id=?", (group_id,))
        top_likes = (await cursor.fetchone())[0]
        await db.execute("UPDATE group_parser SET top_likes=? WHERE id=?", (not top_likes, group_id,))
        await db.commit()


async def change_amount(group_id: int, photo_amount: int):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE group_parser SET photo_amount=? WHERE id=?", (photo_amount, group_id,))
        await db.commit()


async def change_date(group_id: int, date: int):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE group_parser SET time_delta=? WHERE id=?", (date, group_id,))
        await db.commit()


async def change_hour(user_id: int, hour: int):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE admins_info SET hour=? WHERE id=?", (str(hour), user_id,))
        await db.commit()


async def change_minute(user_id: int, minute: str):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE admins_info SET minute=? WHERE id=?", (minute, user_id,))
        await db.commit()


async def add_group(user_id: int, domain: str):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT groups_to_moderate FROM admins_info WHERE id=?", (user_id,))
        ...
        await db.execute("UPDATE admins_info SET groups_to_moderate=? WHERE id=?", (groups, user_id,))
        await db.commit()
        name, photo = api_group_name(domain)
        cursor = await db.execute("INSERT INTO group_parser (id, group_domain, active, top_likes, photo_amount, time_delta, last_update, name, cover) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
async def update_time(group_id: int, time_now=None):
    async with aiosqlite.connect(adminDB) as db:
        ...
        await db.execute("UPDATE group_parser SET last_update=? WHERE id=?", (time_now, group_id))
        await db.commit()


async def delete_group(group_id: int, user_id: int):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute('DELETE FROM group_parser WHERE id=?', (group_id,))
        await db.commit()
        cursor = await db.execute("SELECT groups_to_moderate FROM admins_info WHERE id=?", (user_id,))
...
async def last_group_photo_id():
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT MAX(id) FROM group_photos")
        rows = (await cursor.fetchone())[0]
        if rows is None:
            return 0
        return rows
...
async def add_group_photo(num, origin, caption, url):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("INSERT INTO group_photos (id, group_name, caption, url) VALUES (?, ?, ?, ?)",
...
async def del_group_photo(num):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute('DELETE FROM group_photos WHERE id=?', (num,))
        await db.commit()


async def get_hour(user_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT hour FROM admins_info WHERE id=?", (user_id,))
        rows = await cursor.fetchone()
        return int(rows[0])
...
async def set_hour(user_id: int, hour: str):
    async with aiosqlite.connect(adminDB) as db:
        await db.execute("UPDATE admins_info SET hour=? WHERE id=?", (hour, user_id,))
        await db.commit()


async def get_minute(user_id: int):
    async with aiosqlite.connect(adminDB) as db:
        cursor = await db.execute("SELECT minute FROM admins_info WHERE id=?", (user_id,))
        rows = await cursor.fetchone()
        return int(rows[0])
